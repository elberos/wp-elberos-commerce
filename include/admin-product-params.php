...
class Product_Params_Table extends \WP_List_Table 
{
	...
	function column_name($item)
	{
		...
		$text_name = isset($name["ru_RU"]) ? $name["ru_RU"] : "";
		return $text_name;
	}
	...
	function prepare_items()
	{
		...
		if ($order == "" && $orderby == ""){ $orderby = "alias"; $order = "asc"; }
	...
	function process_item($item)
	{
		...
		$name_en = isset($_POST["params_name"]["en_US"]) ? $_POST["params_name"]["en_US"] : "";
		$name_ru = isset($_POST["params_name"]["ru_RU"]) ? $_POST["params_name"]["ru_RU"] : "";
		if ($item["alias"] == "")
		{
			if ($name_en) $item["alias"] = sanitize_title($name_en);
			else if ($name_ru) $item["alias"] = sanitize_title($name_ru);
		}
		...
		{
			$locale = $lang["locale"];
			$name = isset($_POST["params_name"][$locale]) ? $_POST["params_name"][$locale] : "";
	...
	function display_form($item)
	{
		...
		foreach ($langs as $key => $lang)
		{
			$text_name = isset($name[$lang['locale']]) ? $name[$lang['locale']] : "";
			...
			<br>
				<input id="name[<?= esc_attr($lang['locale']) ?>]" 
					name="params_name[<?= esc_attr($lang['locale']) ?>]"
